...
for i in range(3):
    ...
    display.text("Health: Rel PS5 Rel 24302", COL_TAB, row+ROW_SPACING+ROW_BIAS-2, scale=INFO_SCALE)
    display.text("Deploy-Playstation", COL_TAB, row+ROW_SPACING+ROW_BIAS+row_info_height, scale=INFO_SCALE)
    display.set_pen(FG_BODY)
    display.text("1:33:00", COL_TAB_2, row+ROW_SPACING+ROW_BIAS+row_info_height, scale=INFO_SCALE)
    row += row_step
...
display.circle(10 + status_circle_radius, row_status_circle_center, status_circle_radius)
display.text("Health: PS4 Rel 20220", 10+status_circle_radius*2+10, row_status_circle_center-8, scale=INFO_SCALE)
